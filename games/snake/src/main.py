...
class SnakeCashRush:
    def __init__(self) -> None:
        """Initialize DOM bindings, persistent score state, and game event hooks."""
    ...
    def reset_state(self) -> None:
        """Reset the game to its initial idle state and update HUD labels."""
    ...
    def start_game(self) -> None:
        """Start or resume a run, reinitializing timing fields as needed."""
        if self.running:
            return
        if self.game_over:
            self.reset_state()
    ...
    def handle_primary_action(self) -> None:
        """Handle the primary button action, toggling start or restart behavior."""
    ...
    def restart_game(self) -> None:
        """Fully restart the run from a clean state."""
        self.cancel_animation()
        self.reset_state()
        self.start_game()

    def ensure_animation(self) -> None:
        """Schedule a new animation frame when no frame is currently pending."""
        if self.animation_handle is None:
            self.animation_handle = window.snakeCashRushBridge.raf(self._frame_proxy)

    def cancel_animation(self) -> None:
        """Cancel the active animation frame callback if one exists."""
    ...
    def set_overlay(
        ...
        """Update overlay text and visibility to reflect the current game state."""
    ...
    def handle_keydown(self, event) -> None:
        ...
        # Prevent 180-degree turns to avoid instant self-collision artifacts.
    ...
    def is_reverse(self, next_direction: Point, current_direction: Point) -> bool:
        """Return whether next_direction would reverse current movement direction."""
        return next_direction.x == -current_direction.x and next_direction.y == -current_direction.y

    def game_frame(self, timestamp) -> None:
        """Advance simulation by elapsed time and render the current frame."""
        self.animation_handle = None
        ...
        self.accumulator += frame_delta
                ...
                # Catch up using fixed simulation ticks so game speed stays consistent
                # even when frame rate fluctuates across devices.
    ...
    def advance(self) -> None:
        """Execute one game tick: move snake, resolve collisions, and score updates."""
        self.direction = self.pending_direction
        head = self.snake[-1]
        next_head = Point(head.x + self.direction.x, head.y + self.direction.y)
        will_collect = next_head == self.cash
        # When not collecting cash, the tail moves away this tick, so allow
        # stepping into the previous tail cell without counting it as a hit.
        ...
        if will_collect:
            ...
            # Gradually increase pace per pickup, but cap at a playable minimum.
    ...
    def hit_wall(self, point: Point) -> bool:
        """Return True when a point lies outside the board boundaries."""
    ...
    def spawn_cash(self, snake: Iterable[Point]) -> Point:
        """Spawn cash on a random unoccupied board cell."""
        occupied = set(snake)
        # Build from all valid cells so placement is always fair and collision-free.
        available = [
            Point(x, y)
    ...
    def sync_best_score(self) -> None:
        """Persist and animate best-score changes when a new record is reached."""
    ...
    def flash_score(self) -> None:
        """Trigger a brief pulse animation on the current score tile."""
    ...
    def show_cash_burst(self) -> None:
        """Show a short visual burst when the snake collects cash."""
    ...
    def end_game(self) -> None:
        """Stop the run and display final score messaging in the overlay."""
        self.running = False
        self.game_over = True
    ...
    def draw(self) -> None:
        """Render the board, cash pickup, and snake for the current state."""
        ctx = self.ctx
    ...
    def draw_cash(self, ctx) -> None:
        """Draw the cash pickup token on its current grid cell."""
    ...
    def draw_snake(self, ctx) -> None:
        """Draw snake body segments and head details."""
        cell = BOARD_PIXELS / BOARD_CELLS
    ...
    def snapshot_json(self) -> str:
        """Return a JSON snapshot of the current game state for debugging."""
        payload = {
    ...
    def place_cash_ahead(self) -> None:
        """Move cash to a nearby safe tile to speed up manual testing."""
        head = self.snake[-1]
        candidates = [
    ...
    def step_debug(self) -> None:
        """Advance one simulation step for debug workflows without full animation."""
        if self.game_over:
            return
        self.running = True
        self.advance()
    ...
    def destroy(self) -> None:
        """Clean up animation and event listeners when tearing down the game."""
        self.cancel_animation()
...
def round_rect(ctx, x: float, y: float, width: float, height: float, radius: float) -> None:
    """Build a rounded-rectangle path on the provided canvas context."""
    ctx.beginPath()
